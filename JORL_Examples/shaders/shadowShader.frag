...
float calculateShadow(){
	vec4 projectedPosition = light_matrix*vec4(ModelPosition, 1.0);
	vec4 mvPosition = light_mv_matrix*vec4(ModelPosition, 1.0);

	vec2 uv = (projectedPosition.xy/projectedPosition.w)*0.5 + vec2(0.5, 0.5);

	float costheta = dot(normalize(FragmentNormal), normalize(lightPos - ModelPosition));
	float bias = tan(acos(costheta))*0.005;

	vec2 distTex = texture(depthTexture, uv).rg;
	float varianceSquared = distTex.g - distTex.r*distTex.r;

	float depth = -mvPosition.z;
	float meanShadowDepth = -distTex.r;

	if(meanShadowDepth > depth - bias)
		return 1.0;

	float visibility = varianceSquared/(varianceSquared + pow(depth - bias - meanShadowDepth, 2.0));

	return clamp(visibility*visibility, 0, 1);
//	return 0.0;
}
...
*/
	...
	float lambertion = clamp(dot(normal, light), 0.0, 1.0)*visibility;
